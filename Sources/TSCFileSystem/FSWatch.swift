// SPDX-FileCopyrightText: 2014-2017 Apple Inc, 2017-2025 Swift Project
...
public class FSWatch {
    ...
    public init(paths: [AbsolutePath], latency: Double = 1, block: @escaping EventReceivedBlock) {
        ...
        for path: AbsolutePath in paths {
            var cbool: CBool = false
            if !FileManager.default.fileExists(atPath: path.pathString, isDirectory: &cbool) {
                // FIXME: Report errors
                continue
            }
...
public final class RDCWatcher {
    class Watch {
        ...
        var thread: TSCFileSystem.Thread?
...
public final class Inotify {
    ...
    private func startReportThread() {
        let thread = TSCFileSystem.Thread {
            var endLoop = false
            while !endLoop {
...
public final class FSEventStream {
    ...
    // Start the runloop.
    public func start() throws {
        let thread = TSCFileSystem.Thread { [weak self] in
